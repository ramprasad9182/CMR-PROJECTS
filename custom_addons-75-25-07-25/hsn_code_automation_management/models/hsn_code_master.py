...
class HsnCodeMaster(models.Model):
    ...
    sale_tax_id = fields.Many2one("account.tax", string="GST Sales")
    purchase_tax_id = fields.Many2one("account.tax", string="GST Purchase")
    igst_sale_tax_id = fields.Many2one("account.tax", string="GST Sales(IGST)")
    igst_purchase_tax_id = fields.Many2one("account.tax", string="GST Purchase(IGST)")
    ...
    def _create_hsn_code_with_tax(self, hsn_category, company_id):
        ...
        if hsn_category == "99":
            ...
            url = (
                f"{COMMON_URL}{hsn_category}-{hsn_category_selection}-gst-rate-hsn-code"
            )
            ...
            if table:
                ...
                for row in rows:
                    ...
                    if len(rec) >= 3:
                        sale_tax_id = (
                            igst_sale_tax_id
                        ) = purchase_tax_id = igst_purchase_tax_id = None
                        ...
                        if tax_rate == "":
                            ...
                            for record in nil_tax:
                                if record.type_tax_use == "sale":
                                    igst_sale_tax_id = sale_tax_id = record.id
                                elif record.type_tax_use == "purchase":
                                    igst_purchase_tax_id = purchase_tax_id = record.id
                        else:
                            tax_rate = float(tax_rate.replace("%", ""))
                        ...
                        value = {
                            ...
                            "igst_sale_tax_id": igst_sale_tax_id,
                            "purchase_tax_id": purchase_tax_id,
                            "igst_purchase_tax_id": igst_purchase_tax_id,
