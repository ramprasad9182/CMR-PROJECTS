...
class AccountOutPayment(models.Model):
    ...
    def action_out_payment(self):
        """This method constructs an XML request to synchronize Out Payment
        ...
        head_xml = '<ENVELOPE>\
            ...
            <VOUCHER REMOTEID="" VCHKEY="" VCHTYPE="Payment" ACTION="Create" OBJVIEW="Accounting Voucher View">' % (company)
        xml_foot = '</VOUCHER>\
                    </TALLYMESSAGE>\
                    </REQUESTDATA>\
                    </IMPORTDATA>\
        ...
        body_list = []
        line_items = []
        for partner_line in self.line_ids.filtered(lambda l:
                                                   l.account_type == 'Receivable' and l.account_id):
