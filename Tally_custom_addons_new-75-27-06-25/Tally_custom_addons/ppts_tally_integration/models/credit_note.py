...
class CreditNote(models.Model):
    ...
    def action_credit_note(self):
        ...
        if self.invoice_date:
            ...
            date_invoice = str(self.invoice_date.year) + month + day

        if self.invoice_date_due:
            day = '0' + str(self.invoice_date_due.day) if self.invoice_date_due.day < 10\
                else str(self.invoice_date_due.day)
            month = '0' + str(self.invoice_date_due.month) if self.invoice_date_due.month < 10 \
                else str(self.invoice_date_due.month)
            # date_due = str(self.invoice_date_due.year) + month + day
        ...
        parent_xml = '<DATE>%s</DATE>\
            ...
            date_invoice, str(self.id), sync_date, self.name,
        ...
        for rec in self.invoice_line_ids:
            ...
            <AMOUNT>%s</AMOUNT>\
                    ...
                    rec.price_subtotal,rec.quantity,rec.quantity,rec.price_subtotal ,
                    rec.quantity,rec.quantity,date_invoice,rec.account_id.name,rec.price_subtotal)
            ...
            if rec.account_id.id not in account_coa:
                if rec.debit > 0.0:
                    ...
                    amount_tax = ('<LEDGERENTRIES.LIST>\
                                  ...
                                  (rec.account_id.name, rec.debit, rec.debit))
