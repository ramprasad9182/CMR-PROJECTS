...
class APIController(http.Controller):
    """ This class defines an API controller with routes for creating credit note.
    ...
    @validate_token
    @http.route("/api/create/credit_note", type="json", auth="public", methods=["POST"], csrf=False)
    def _api_create_credit_note(self):
        ...
        if 'credit_note' not in popup_data:
            _logger.info('@The Tally system does not contain any credit note.')
            return
        request.env['tally.entries'].sudo().create({
            'number_of_entries': len(popup_data['credit_note']),
            'tally_data': popup_data,
            'tally_entry_type': 'out_refund'
        })
        ...
        _logger.info('@ Total Received data to create credit note from Tally to Odoo: %s',
                     popup_data['credit_note'])
        tally_log_ids = []
        credit_count = 0
        for rec in popup_data['credit_note']:
            try:
                tally_credit_ids = request.env['account.move'].sudo().search(
                    [('tally_credit_id', '=', rec['tally_credit_id'])])
                customer_id = request.env['res.partner'].sudo().search([
                    ('tally_id', '=', int(rec['partner_id']))], limit=1)
                if customer_id and not tally_credit_ids:
                    line_item = []
                    for line in rec['invoice_line_ids']:
                        ...
                        if 'tax_ids' in line:
                            for tax in line['tax_ids']:
                                tax_name = tax['name'] + ' ' + str(tax['amount']) + '%' + ' ' + '(' + str(
                                    tax['emirate']) + ')'
                        ...
                        if line['is_has_product'] == 'Yes':
                            ...
                            line_item.append(vals)
                    if line_item:
                        res_currency_id = request.env['res.currency'].sudo().search([('id', '=', rec['currency_id'])],
                                                                                    limit=1)
                        ...
                        credit_note = request.env['account.move'].sudo().create({
                            ...
                            'ref': rec['ref'],
                            'tally_credit_id': rec['tally_credit_id'],
                            'tally_credit_name': rec['tally_credit_name'],
                            # 'currency_id': res_currency_id.id if res_currency_id else False,
                            'invoice_line_ids': line_item
                        })
                        credit_note.write({'picking_type_id': self._get_stock_type_ids(credit_note)})
                        credit_note.action_post()
                        credit_note.action_stock_receive()
                        credit_count += 1
                        if module.state == 'installed':
                            vals = (0, 0, {
                                'master_type': 'out_refund',
                                'sync_action': 'create',
                                'sync_data': str(rec),
                                'error_data': 'Credit Note has been created',
                                'name': credit_note.name,
                                'sync_status': 'done',
                                'sync_for': 'trans',
                                'tally_record_name': rec['tally_credit_name'],
                                'records_created_id': credit_note.id,
                                'tally_record_id': rec['tally_credit_id']
                            })
                ...
                if module.state == 'installed':
                    ...
                    tally_log_ids.append(vals)
        _logger.info('@ Number of created credit note from Tally to Odoo: %s', credit_count)
        if tally_log_ids:
            vals = {
                'data_from': 'tally',
