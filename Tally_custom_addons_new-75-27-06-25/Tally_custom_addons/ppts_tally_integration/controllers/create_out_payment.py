...
class APIController(http.Controller):
    ...
    # def out_payment_reconcile_add(self, payments, rec):
    #     """."""
    #     for line in list(rec['tally_invoice_ids']):
    ...
    #             ('tally_invoice_id', '=', line),
    #             ('payment_state', '=', 'not_paid'), ('state', '=', 'posted')])
    #         credit_aml = payments.invoice_line_ids.filtered('credit')
    #         if (credit_aml.credit < account_move.amount_residual and
    #                 len(list(rec['tally_invoice_ids'])) != 1):
    #             _logger.info('@ ODOO Customer Payment Record: %s and Tally Customer Payment Record: %s',
    #                          payments.name, payments.tally_payment_name)
    #             continue
    #         if account_move:
    #             account_move.js_assign_outstanding_line(credit_aml.id)
    #             _logger.info('@ Odoo customer invoice %s is reconcile with %s payment.',
    #                          account_move.name, payments.name)
    #         else:
    #             _logger.info('@ The customer invoice in Tally has not been updated to '
    #                          'match the invoice record in Odoo.')

    def out_payment_reconcile_add(self, payments, rec):
        """The sale receipt are reconcile with the tally invoice voucher number."""
        data = []
        ...
        if not amount > payments.amount:
            for line in list(rec['tally_invoice_ids']):
                account_move = request.env['account.move'].sudo().search([
                    ('tally_invoice_name', '=', line['tally_name']),
                ...
                data.append((0, 0, {
                    ...
                    'account_id': account_move.partner_id.property_account_receivable_id.id,
        ...
        print("payments", [pay.ids for pay in payments.dev_invoice_line_ids])
        payments.sudo().action_post()

    @validate_token
    @http.route("/api/create/out_payments", type="json", auth="public", methods=["POST"],
                csrf=False)
    def _api_create_out_payments(self):
        popup_data = json.loads(request.httprequest.data)
        if 'payments' not in popup_data:
            _logger.info('@The Tally system does not contain any customer payment.')
            return
        request.env['tally.entries'].sudo().create({
            'number_of_entries': len(popup_data['payments']),
            'tally_data': popup_data,
            'tally_entry_type': 'out_payment'
        })
        _logger.info('Importing Customer Payments from tally to odoo %s',
                     popup_data)
        ...
        for rec in popup_data['payments']:
            try:
                ...
                if not payments:
                    ...
                    if module.state == 'installed':
                        vals = (0, 0, {
                            'master_type': 'out_payment',
                            'sync_action': 'create',
                            'sync_data': str(rec),
                            'error_data': 'Customer Payment has been created',
                ...
                if 'tally_invoice_ids' in rec:
                    self.out_payment_reconcile_add(payments, rec)
