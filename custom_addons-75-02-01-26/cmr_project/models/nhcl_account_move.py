...
class AccountMove(models.Model):
            ...
            if record.move_type in ['out_invoice', 'in_invoice', 'out_refund', 'in_refund']:
                ...
                if duplicate:
                    raise ValidationError(
                        f"The reference '{record.ref}' already exists "
                        f"Please enter a unique reference."
                    )

                # PO-related checks
                for invoice_line in record.invoice_line_ids:
                    if invoice_line.purchase_line_id.order_id:
                        product_type = invoice_line.product_id.type
                        if product_type == 'consu':
                            if invoice_line.quantity > invoice_line.purchase_line_id.qty_received:
                                raise ValidationError(
                                    f"You cannot give more than received qty "
                                    f"{invoice_line.purchase_line_id.qty_received}, "
                                    f"given {invoice_line.quantity}"
                                )
                        else:
                            po_qty = invoice_line.purchase_line_id.product_qty
                            if invoice_line.quantity > po_qty:
                                raise ValidationError(
                                    f"You cannot give more than ordered quantity {po_qty}, "
                                    f"given {invoice_line.quantity} for product {invoice_line.product_id.display_name}"
                                )
    ...
    #             ('id', '!=', record.id),
    #             ('state', '=', 'posted'),
    #             ('move_type', '=', record.move_type),  # only same type
    #
    #         ], limit=1)
    #         if duplicate:
    ...
    #                     if invoice_line.quantity > invoice_line.purchase_line_id.qty_received:
    #                         raise ValidationError(
    #                             f"You cannot give more than received qty {invoice_line.purchase_line_id.qty_received}, given {invoice_line.quantity}"
    #                         )
    #                 else:
    #                     po_qty = invoice_line.purchase_line_id.product_qty
    #                     if invoice_line.quantity > po_qty:
...
class AccountPayment(models.Model):
    ...
    def create(self, vals):
        ...
        """
        if 'nhcl_purchase_id' in vals and 'amount' in vals:
...
class AccountMoveLine(models.Model):
    ...
    #     for rec in self:
    #         # Only real invoice/bill product lines
    #         valid_lines = rec.move_id.line_ids.filtered(lambda l: l.display_type == 'product')
    ...
    def _compute_s_no(self):
        for rec in self:
            ...
            # Only product lines (ignore sections, notes, terms lines)
            valid_lines = move.line_ids.filtered(lambda l: l.display_type == 'product')

            # Assign S.No based on natural order (safe for NewId)
            for index, line in enumerate(valid_lines, start=1):
                if line == rec:  # <<< USE THIS
                    rec.s_no = index
                    break
            else:
