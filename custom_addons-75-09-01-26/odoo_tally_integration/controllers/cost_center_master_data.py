            ...
            if odoo_id:
                ...
                if not cos_center:
                    ...
                    if existing_center.exists():
                        if existing_center.tally_record_id == tally_id:
                            if existing_center.nhcl_tally_flag == 'y':
                                result = json.dumps({
                                    'status': 'success',
                                    'message': f'{int(odoo_id)} Id CostCenter Record Create Flag Already Updated successfully'
                                })
                            else:
                                result = json.dumps({
                                    'status': 'info',
                                    'message': f'{int(odoo_id)} Id CostCenter Record Create Flag value invalid'
                                })
                        else:
                            result = json.dumps({
                                'status': 'info',
                                'message': f'{int(odoo_id)} Id CostCenter Record Tally Id Not Found'
                            })
                    else:
                        result = json.dumps({
                            'status': 'info',
                            'message': f'{int(odoo_id)} Id CostCenter Record Not Found'
                        })
                else:
                    for je in cos_center:
                        ...
                        result = json.dumps({
                            'status': "success",
                            'message': f'{int(odoo_id)} Id Record CostCenter Create Flag updated successfully'
                        })
